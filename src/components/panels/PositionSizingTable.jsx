  ...
  ({ metrics, capital, lotSize, riskMode, riskPerTrade, riskPercent }) => (
    <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
      ...
      <p className="text-xs text-gray-600 mb-3">
        Based on Kelly criterion and risk per trade. Capital levels in multiples
        of current.
      </p>
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-gray-800 text-gray-500">
              ...
              <th className="text-center py-2 px-3 font-semibold">Max Lots</th>
              <th className="text-center py-2 px-3 font-semibold">
                Kelly Half Lots
              </th>
              <th className="text-center py-2 px-3 font-semibold">
          ...
          <tbody>
            {[0.5, 1, 2, 5, 10].map((mult) => {
              ...
              const risk =
                riskMode === "fixed" ? riskPerTrade : cap * (riskPercent / 100);
              const kellyPct = Math.max(0, +metrics.kellyHalf) / 100;
              const kellyRisk = cap * kellyPct;
              const maxLots =
                lotSize > 0 ? Math.floor(risk / (lotSize * 10)) || 1 : 1;
              const kellyLots =
                lotSize > 0 && kellyPct > 0
                  ? Math.max(1, Math.floor(kellyRisk / (lotSize * 10)))
                  : 1;
      ...
      <div className="text-xs text-gray-600 mt-2">
        Lot size: {lotSize} | Kelly Half: {metrics.kellyHalf}% | Conservative =
        ½ of max
      </div>
    </div>
  ),
