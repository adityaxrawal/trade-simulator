    ...
    return (
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-3">
        <KPICard
          ...
          value={
            isCrypto
              ? formatCrypto(metrics.netPnL / usdToInr)
              : formatINR(metrics.netPnL)
          }
          subText={
            isCrypto
              ? `Gross: ${formatCrypto(metrics.grossPnL / usdToInr)}`
        ...
        <KPICard
          ...
          isNegative={
            isFinite(metrics.profitFactor) && metrics.profitFactor < 1
          }
          icon={Activity}
        />
        <KPICard
          ...
          value={
            isCrypto
              ? formatCrypto(metrics.expectancy / usdToInr)
        ...
        <KPICard
          ...
          subText={
            isCrypto
              ? formatCrypto(metrics.maxDrawdownRs / usdToInr, 0)
        ...
        <KPICard
          ...
          value={
            isCrypto
              ? formatCrypto(metrics.totalCharges / usdToInr)
        ...
        />
        <KPICard
          label="Sharpe Ratio (Per Trade)"
          value={
            !isFinite(metrics.perTradeSharpe)
              ? metrics.perTradeSharpe < 0
                ? "-∞"
                : "∞"
              : metrics.perTradeSharpe.toFixed(2)
          }
          subText={`Per Trade`}
          tooltip="Sharpe Ratio (Per Trade)"
          isPositive={metrics.perTradeSharpe > 0.05}
          isNegative={metrics.perTradeSharpe < 0}
          icon={Activity}
        />
        <KPICard
        ...
        <KPICard
          ...
          value={
            isCrypto
              ? formatCrypto(metrics.finalCapital / usdToInr)
